...
static void update_pes_stream_type(TRANSPORT_STREAM *ts);
...
NOT_FOUND_PAT_PMT:
	while( (p = read_unit(ts)) != NULL ){
		...
		}else if( (header.adaptation_field_control & 1) && header.payload_unit_start_indicator ){
			if(packets[header.pid] == NULL){
				packets[header.pid] = (PES_PACKET *)malloc(sizeof(PES_PACKET));
				init_pes_packet(packets[header.pid]);
			}
...
static void update_pes_stream_type(TRANSPORT_STREAM *ts)
{
	...
	offset = ts->offset;
	offset += ts->unit_size * (ts->file_length / (ts->unit_size * 8));
	ts_seek_raw(ts, offset, SEEK_SET);

	read_pes_packet(ts);
	extract_pes_stream_type(&(ts->packet), &type);
